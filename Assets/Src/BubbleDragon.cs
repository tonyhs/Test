...
public static Color[] ColorPalette = new Color[] {
        new Color(0,189/255f,213/255f) ,
         new Color(220/255f,90/255f,56/255f) ,
         new Color(108/255f,147/255f,216/255f) ,
         new Color(80/255f,210/255f,166/255f) ,
         new Color(250/255,187/255f,48/255f) ,
         new Color(226/255f,163/255f,161/255f) ,
          new Color(205/255f,144/255f,211/255f) ,
           new Color(118/255f,201/255f,83/255f) ,
            new Color(155/255f,111/255f,200/255f) ,
             new Color(118/255f,80/255f,163/255f) ,
              new Color(227/255f,129/255f,56/255f) ,
    };
    ...
    static void SetColor( Material mat, int colorIndex ) {
        if ( mat != null ) {
           // sprite = currentObjContainer.transform.GetChild(0);
    ...
    bool _CheckDeadline() {
        ...
        if ( m_buffer.Count > m_deadLine ) {
          ...
          //  lines.transform.position = new Vector3(lines.transform.position.x, lines.transform.position.y + 0.20f, lines.transform.position.z);
            Debug.Log("Bot");
        }
        return count != 0;
    }
    ...
    void DoEliminateBubbles( List<BubbleCell> eraseList ) {
        for ( int i = 0; i < eraseList.Count; ++i ) {
            var b = eraseList[ i ];
            if (i == eraseList.Count - 1)
            {
                Debug.Log("so");
                if (b.mat.color == new Color(0, 189 / 255f, 213 / 255f))
                {
                    Debug.Log(GetBubbleColor( eraseList[i].x, eraseList[i].y ));
                    b.mat.color = new Color(220 / 255f, 90 / 255f, 56 / 255f);
                    b.color = 1;
                    eraseList[i].color = 2;
                    Debug.Log(GetBubbleColor(eraseList[i].x, eraseList[i].y));

                }
                else if (b.mat.color == new Color(220 / 255f, 90 / 255f, 56 / 255f))
                {
                    Debug.Log("so");
                    b.mat.color = new Color(108 / 255f, 147 / 255f, 216 / 255f);
                    b.color = 2;
                    eraseList[i].color = 3;
                }
                else if (b.mat.color == new Color(108 / 255f, 147 / 255f, 216 / 255f))
                {
                    Debug.Log("so");
                    b.mat.color = new Color(80 / 255f, 210 / 255f, 166 / 255f);
                    b.color = 3;
                    eraseList[i].color = 4;
                }
                else if(b.mat.color == new Color(80 / 255f, 210 / 255f, 166 / 255f))
                {
                    Debug.Log("so");
                    b.mat.color = new Color(250 / 255, 187 / 255f, 48 / 255f);
                    b.color = 4;
                    eraseList[i].color = 5;
                }
                else if(b.mat.color == new Color(250 / 255, 187 / 255f, 48 / 255f))
                {
                    Debug.Log("so");
                    b.mat.color = new Color(226 / 255f, 163 / 255f, 161 / 255f);
                    b.color = 5;
                    eraseList[i].color = 6;
                }
                if (b.mat.color == new Color(0, 189 / 255f, 213 / 255f))
                ...
                else if (b.mat.color == new Color(226 / 255f, 163 / 255f, 161 / 255f))
                {
                    score = score + 64;
                }
                else if (b.mat.color == new Color(205 / 255f, 144 / 255f, 211 / 255f))
                {
                    score = score + 128;
                }
                else if (b.mat.color == new Color(118 / 255f, 201 / 255f, 83 / 255f))
                {
                    score = score + 256;
                }
                else if (b.mat.color == new Color(227 / 255f, 129 / 255f, 56 / 255f))
                {
                    score = score + 512;
                }
                else if (b.mat.color == new Color(155 / 255f, 111 / 255f, 200 / 255f))
                {
                    score = score + 1024;
                }
                else if (b.mat.color == new Color(118 / 255f, 80 / 255f, 163 / 255f))
                {
                    score = score + 2048;
                    gameObj = GameObject.FindGameObjectsWithTag("Ballon");
                    for (int j = 0; i < gameObj.Length; i++)
                        Destroy(gameObj[i]);
                }
                AudioClip clip = (AudioClip)Resources.Load("Ballon-pop");
                audio.GetComponent<AudioSource>().PlayOneShot(impact,0.25f);
                m_eraseList.Remove(b);
                progress.fillAmount += score / LvlScore;
   ...
   void DoDropDestroyBubbles( List<BubbleCell> eraseList ) {
       for ( int i = 0; i < eraseList.Count; ++i ) {
           var b = eraseList[i];
            /* if (i == eraseList.Count-1)
             {
                 Debug.Log("so");
                 if (eraseList[i].mat.color == Color.red)
                 {
                     Debug.Log("so");
                     eraseList[i].mat.color = Color.green;
                 }
        ...
        if ( !bubble.stopped ) {
            ...
            if ( bubble.stopped ) {
                ...
                if ( slot != null ) {
                    if ( !_CheckDeadline() ) {
                        try {
                            CheckEliminate( new iPoint( slot.x, slot.y ), slot.color, m_eraseList, 2 );
    ...
    private float spaceTimer = 20f;
    public Image progress;
    public float LvlScore = 10;
    public Text currLvl;
    public Text nextLvl;
    public int currLvln = 1;
    public int nextLvln = 2;
    ...
    void Update() {
        ...
        if(progress.fillAmount == 1)
        {
            ...
            StartCoroutine(Wait());
            LvlScore = LvlScore + UnityEngine.Random.Range(100, 500);
        }
        currLvl.text = currLvln.ToString();
        nextLvl.text = nextLvln.ToString();
        ...
        {
            ScoreTxt.text = (score / 100).ToString() + "k";
        }
        else
        {
            ScoreTxt.text = (score / 100000).ToString() + "m";
        }
    ...
    spaceTimer -= Time.deltaTime;
        ...
        if ( m_shootBubbleState == 0 ) {
            ...
            m_shootBubbleColor = UnityEngine.Random.Range( 1, 6 );
